...
void SalOp::updateRemoteFromRequest() {
	...
	if (transaction) {
		...
		if (fromUri) {
			auto fromUriStr = belle_sip_object_to_string(fromUri);
			setRemote(fromUriStr);
			ms_free(fromUriStr);
		}
		if (mDialog) {
...
void SalOp::updateFromHeader(belle_sip_request_t *request, const std::string &method) {
	belle_sip_header_from_t *from = createFromHeader(method, true);
...
belle_sip_header_from_t *SalOp::createFromHeader(const std::string &method, bool isUpdate) const {
	...
	if (useAnonymousFromInRequest(method)) {
		...
		const auto uri = isUpdate ? getLocalUri() : getFromAddress();
		fromHeader = belle_sip_header_from_create(BELLE_SIP_HEADER_ADDRESS(uri), tag.c_str());
	}
	return fromHeader;
}
...
belle_sip_request_t *SalOp::buildRequest(const string &method) {
	...
	belle_sip_header_from_t *fromHeader = createFromHeader(method, false);
