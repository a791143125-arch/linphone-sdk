...
MediaSessionParams *ServerConference::getDefaultMediaParams(const std::shared_ptr<Call> &call,
	...
	setMediaDirectionInCallParams(msp, device);
	return msp;
}
...
bool ServerConference::finalizeParticipantAddition(std::shared_ptr<Call> call) {
	...
	if (device) {
		...
		if (contactAddress && contactAddress->isValid() && !contactAddress->hasParam(Conference::kIsFocusParameter)) {
			if (call == getCore()->getCurrentCall()) {
				L_GET_PRIVATE_FROM_C_OBJECT(getCore()->getCCore())->setCurrentCall(nullptr);
			}
			getCore()->doLater([this, call, device] {
				lInfo() << *this << ": Finalizing addition of " << *device;
				auto params = getDefaultMediaParams(call, device);
				call->update(params);
				delete params;
			});
		}
...
bool ServerConference::addParticipant(const std::shared_ptr<Call> call) {
	...
	if (canAddParticipant) {
		...
		// enters the conference, in fact parameter isfocus is added later on (based on local parameters)
		...
		lInfo() << *call << " is added to " << *this << " while in state " << Utils::toString(state);

		std::shared_ptr<ParticipantDevice> device = findParticipantDevice(session);
		...
		switch (state) {
			...
			case CallSession::State::IncomingEarlyMedia: {
				...
				device = findParticipantDevice(session);
				if (device) {
					auto db = getCore()->getDatabase();
		...
		auto updateFunction = [this, call, session] {
			lInfo() << "Update " << *call << " because it was added to " << *this;
			const std::shared_ptr<Address> &conferenceAddress = getConferenceAddress();
			auto op = call->getOp();
			if (op && conferenceAddress) {
				op->setLocalUri(conferenceAddress->getImpl());
		...
		switch (state) {
			...
			case CallSession::State::Resuming: {
				if (contactAddress && contactAddress->isValid() &&
					...
					getCore()->doLater(updateFunction);
				}
			} break;
			default:
				lError() << *call << " is in state " << Utils::toString(state)
...
int ServerConference::removeParticipant(const std::shared_ptr<CallSession> &session, const bool preserveSession) {
	...
	// Restore previous from address as the call session is now detached from the conference
	auto op = session->getPrivate()->getOp();
	if (op) {
		op->setLocalUri(session->getLocalAddress()->getImpl());
	}
	...
	if (supportsMedia()) {
		...
		switch (state) {
			...
			case CallSession::State::IncomingReceived: {
				...
				bool deviceJoining =
				    ((!Conference::isTerminationState(getState())) && !isCancelled &&
						...
						if (participant && remoteContactAddress) {
							...
							if (hasCallContactChanged) {
								...
								lInfo() << *this << ": Updating address of participant device [" << device
								        << "] with session " << device->getSession() << " from " << *deviceAddr
								        << " to " << *newDeviceAddress;
...
void ServerConference::acceptSession(const shared_ptr<CallSession> &session) {
	...
	const auto ms = static_pointer_cast<MediaSession>(session);
	const auto device = findParticipantDevice(session);
	auto msp = getDefaultMediaParams(nullptr, device);
	if (session->getState() == CallSession::State::UpdatedByRemote) ms->acceptUpdate(msp);
	else ms->accept(msp);
	delete msp;
}
