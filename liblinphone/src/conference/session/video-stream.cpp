...
void MS2VideoStream::render(const OfferAnswerContext &ctx, CallSession::State targetState) {
	...
	if (basicChangesHandled) {
		...
		if (!cameraChanged && !localScreenSharingChanged && !displayModeChanged) {
	...
	if (getCCore()->video_background_path)
		video_stream_set_background_path(mStream, getCCore()->video_background_path);
