...
public:
	...
	static void setWindowId(std::shared_ptr<linphone::Core> core,
	                        ...
	                        bool preview) {
		if (!*windowId)
			*windowId = preview ? core->createNativePreviewWindowId(context) : core->createNativeVideoWindowId(context);
		ogl_set_window_title(*windowId, title.c_str());
...
*/
...
#else
	void *screenSharerContext, *screenSharerPreviewContext;
	void *paulineContext, *paulinePreviewContext;
	void *marieContext, *mariePreviewContext;
#endif
	...
	WrapperTools::setWindowId(screenSharer, &screenSharerPreviewContext, &sharerPreviewWindowId, "Berthe Preview",
	                          true);
