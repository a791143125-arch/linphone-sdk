...
static void edit_simple_conference_base(bool_t from_organizer,
	...
	{ // to make sure focus is destroyed after clients.
		...
		LinphoneMediaEncryption encryption =
		    (enable_encryption) ? LinphoneMediaEncryptionZRTP : LinphoneMediaEncryptionNone;
		...
		if (join) {
			...
			check_call_establishment({focus, marie, pauline, laure, michelle}, {pauline.getCMgr()},
			                         std::make_pair(focus.getCMgr(), focus_stat), member_stats_list, marie.getCMgr(),
			                         confAddr, LinphoneConferenceSecurityLevelNone, encryption, enable_chat, FALSE,
			                         TRUE);
	...
	{ // to make sure focus is destroyed after clients.
		...
		if (!!join) {
			...
			check_call_establishment({focus, marie, pauline, laure, michelle}, {pauline.getCMgr()},
			                         ...
			                         LinphoneMediaEncryptionNone, TRUE, FALSE, TRUE);
...
static void conference_edit_after_leaving(void) {
	...
	{ // to make sure focus is destroyed after clients.
		...
		ms_message("%s is trying to remove participant %s from conference %s",
				...
				if (cr) {
					...
					if (BC_ASSERT_PTR_NOT_NULL(original_content)) {
						...
						if (BC_ASSERT_PTR_NOT_NULL(conf_info_from_original_content)) {
							...
							check_conference_info_members(
							    conf_info_from_original_content, uid, confAddr, marie.getCMgr()->identity,
							    participants_info, start_time, duration, subject, description2, ics_sequence, exp_state,
							    LinphoneConferenceSecurityLevelNone, FALSE, TRUE, video_enabled, chat_enabled);
							linphone_conference_info_unref(conf_info_from_original_content);
						}
					}
		...
		for (auto mgr : {focus.getCMgr(), marie.getCMgr(), laure.getCMgr(), pauline.getCMgr(), michelle.getCMgr()}) {
			...
			if (BC_ASSERT_PTR_NOT_NULL(info)) {
				...
				check_conference_info_in_db(mgr, NULL, confAddr, marie.getCMgr()->identity, participants_info,
				                            start_time, duration, exp_subject, exp_description, exp_sequence, exp_state,
				                            LinphoneConferenceSecurityLevelNone, FALSE, TRUE, video_enabled,
				                            chat_enabled);
			}
			if (info) {
				linphone_conference_info_unref(info);
