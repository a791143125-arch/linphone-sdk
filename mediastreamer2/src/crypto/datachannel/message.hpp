...
struct Message : binary {
	...
	Message(Iterator begin_, Iterator end_, Type type_ = Binary) : binary(begin_, end_), type(type_) {
	}
...
template <typename Iterator>
message_ptr make_message(Iterator begin,
                         Iterator end,
...
template <typename Iterator>
[[deprecated]] message_ptr
make_message(Iterator begin, Iterator end, Message::Type type, unsigned int stream, shared_ptr<FrameInfo> frameInfo) {
	auto message = std::make_shared<Message>(begin, end, type);
	message->stream = stream;
	message->frameInfo = frameInfo;
	return message;
...
message_ptr make_message(size_t size,
                         Message::Type type = Message::Binary,
                         unsigned int stream = 0,
                         shared_ptr<Reliability> reliability = nullptr);

message_ptr make_message(binary &&data,
                         Message::Type type = Message::Binary,
                         unsigned int stream = 0,
                         shared_ptr<Reliability> reliability = nullptr);
