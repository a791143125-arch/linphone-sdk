...
struct DataChannel : Channel, std::enable_shared_from_this<DataChannel> {
	...
	DataChannel(weak_ptr<PeerConnection> pc, string label, string protocol, Reliability reliability);
	virtual ~DataChannel();
...
struct OutgoingDataChannel final : public DataChannel {
	OutgoingDataChannel(weak_ptr<PeerConnection> pc, string label, string protocol, Reliability reliability);
	~OutgoingDataChannel();
