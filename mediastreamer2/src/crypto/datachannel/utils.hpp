...
auto weak_bind(F &&f, T *t, Args &&..._args) {
	return [bound = std::bind(f, t, _args...), weak_this = t->weak_from_this()](auto &&...args) {
		if (auto shared_this = weak_this.lock()) return bound(args...);
		else return static_cast<decltype(bound(args...))>(false);
	};
}
...
public:
	...
	synchronized_stored_callback(CArgs &&...cargs) : synchronized_callback<Args...>(std::forward<CArgs>(cargs)...) {
	}
...
private:
	...
	bool call(Args... args) const {
		if (!synchronized_callback<Args...>::call(args...)) stored.emplace(std::move(args)...);
