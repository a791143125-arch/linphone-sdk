...
extern "C" void ms_sctp_init(void) {
	PLOG_INFO << "sctp init";
	if (!sctp_init_done) {
		SctpTransport::Init();
		sctp_init_done++;
...
extern "C" void ms_sctp_shutdown(void) {
	PLOG_INFO << "sctp shutdown";
	sctp_init_done--;
	if (sctp_init_done == 0) {
...
uint16_t to_uint16(T i) {
	...
	else throw std::invalid_argument("Integer out of range");
}
...
void SctpTransport::changeState(State state) {
	try {
		if (mState.exchange(state) != state) mStateChangeCallback(state);
...
void SctpTransport::Init() {
	usrsctp_init_nothreads(0, SctpTransport::WriteCallback, SctpTransport::DebugCallback);
...
void SctpTransport::SetSettings(const SctpSettings &s) {
	...
	usrsctp_sysctl_set_sctp_delayed_sack_time_default(to_uint32(s.delayedSackTime.value_or(20ms).count()));

	// RTO settings
	// RFC 2988 recommends a 1s min RTO, which is very high, but TCP on Linux has a 200ms min RTO
	usrsctp_sysctl_set_sctp_rto_min_default(to_uint32(s.minRetransmitTimeout.value_or(200ms).count()));
	// Set only 10s as max RTO instead of 60s for shorter connection timeout
	usrsctp_sysctl_set_sctp_rto_max_default(to_uint32(s.maxRetransmitTimeout.value_or(10000ms).count()));
	usrsctp_sysctl_set_sctp_init_rto_max_default(to_uint32(s.maxRetransmitTimeout.value_or(10000ms).count()));
	// Still set 1s as initial RTO
	usrsctp_sysctl_set_sctp_rto_initial_default(to_uint32(s.initialRetransmitTimeout.value_or(1000ms).count()));
	...
	// Heartbeat interval
	usrsctp_sysctl_set_sctp_heartbeat_interval_default(to_uint32(s.heartbeatInterval.value_or(10000ms).count()));
}
...
SctpTransport::SctpTransport(MSDtlsSrtpContext *DtlsContext,
	...
	if (!mSock) throw std::runtime_error("Could not create SCTP socket, errno=" + std::to_string(errno));
	...
	if (usrsctp_setsockopt(mSock, IPPROTO_SCTP, SCTP_RECVRCVINFO, &on, sizeof(on)))
		throw std::runtime_error("Could set socket option SCTP_RECVRCVINFO, errno=" + std::to_string(errno));
	...
	if (usrsctp_setsockopt(mSock, IPPROTO_SCTP, SCTP_EVENT, &se, sizeof(se)))
		throw std::runtime_error("Could not subscribe to event SCTP_ASSOC_CHANGE, errno=" + std::to_string(errno));
	se.se_type = SCTP_SENDER_DRY_EVENT;
	if (usrsctp_setsockopt(mSock, IPPROTO_SCTP, SCTP_EVENT, &se, sizeof(se)))
		throw std::runtime_error("Could not subscribe to event SCTP_SENDER_DRY_EVENT, errno=" + std::to_string(errno));
	...
	if (usrsctp_setsockopt(mSock, IPPROTO_SCTP, SCTP_NODELAY, &nodelay, sizeof(nodelay)))
		throw std::runtime_error("Could not set socket option SCTP_NODELAY, errno=" + std::to_string(errno));
	...
	if (usrsctp_setsockopt(mSock, IPPROTO_SCTP, SCTP_PEER_ADDR_PARAMS, &spp, sizeof(spp)))
		throw std::runtime_error("Could not set socket option SCTP_PEER_ADDR_PARAMS, errno=" + std::to_string(errno));
	...
	if (usrsctp_setsockopt(mSock, IPPROTO_SCTP, SCTP_INITMSG, &sinit, sizeof(sinit)))
		throw std::runtime_error("Could not set socket option SCTP_INITMSG, errno=" + std::to_string(errno));
	...
	if (usrsctp_setsockopt(mSock, IPPROTO_SCTP, SCTP_FRAGMENT_INTERLEAVE, &level, sizeof(level)))
		throw std::runtime_error("Could not disable SCTP fragmented interleave, errno=" + std::to_string(errno));
	...
	if (usrsctp_setsockopt(mSock, IPPROTO_SCTP, SCTP_ACCEPT_ZERO_CHECKSUM, &edmid, sizeof(edmid)))
		throw std::runtime_error("Could set socket option SCTP_ACCEPT_ZERO_CHECKSUM, errno=" + std::to_string(errno));
#endif

	int rcvBuf = 0;
	socklen_t rcvBufLen = sizeof(rcvBuf);
	if (usrsctp_getsockopt(mSock, SOL_SOCKET, SO_RCVBUF, &rcvBuf, &rcvBufLen))
		throw std::runtime_error("Could not get SCTP recv buffer size, errno=" + std::to_string(errno));
	int sndBuf = 0;
	socklen_t sndBufLen = sizeof(sndBuf);
	if (usrsctp_getsockopt(mSock, SOL_SOCKET, SO_SNDBUF, &sndBuf, &sndBufLen))
		throw std::runtime_error("Could not get SCTP send buffer size, errno=" + std::to_string(errno));
	...
	if (usrsctp_setsockopt(mSock, SOL_SOCKET, SO_RCVBUF, &rcvBuf, sizeof(rcvBuf)))
		throw std::runtime_error("Could not set SCTP recv buffer size, errno=" + std::to_string(errno));

	if (usrsctp_setsockopt(mSock, SOL_SOCKET, SO_SNDBUF, &sndBuf, sizeof(sndBuf)))
		throw std::runtime_error("Could not set SCTP send buffer size, errno=" + std::to_string(errno));
...
SctpTransport::~SctpTransport() {
	...
	ms_dtls_srtp_set_recv_cb(mDtlsContext, nullptr);
	// unregisterIncoming();
...
void SctpTransport::connect() {
	PLOG_DEBUG << "SCTP connecting (local port=" << mPorts.local << ", remote port=" << mPorts.remote << ")";
...
bool SctpTransport::send(message_ptr message) {
	...
	if (!message) return trySendQueue();

	PLOG_VERBOSE << "Send size=" << message->size();

	if (message->size() > mMaxMessageSize) throw std::invalid_argument("Message is too large");

	// Flush the queue, and if nothing is pending, try to send directly
	if (trySendQueue() && trySendMessage(message)) return true;
...
void SctpTransport::incoming(message_ptr message) {
	PLOG_ERROR << "JOHAN SctpTransport::incoming";
...
bool SctpTransport::outgoing(message_ptr message) {
	...
	return ms_dtls_srtp_send(mDtlsContext, reinterpret_cast<const unsigned char *>(message->data()), message->size());
}
...
void SctpTransport::doRecv() {
	...
	try {
		while (state() != State::Disconnected && state() != State::Failed) {
			...
			int flags = 0;
			ssize_t len =
			    usrsctp_recvv(mSock, buffer, bufferSize, nullptr, &fromlen, &info, &infolen, &infotype, &flags);
			if (len < 0) {
				if (errno == EWOULDBLOCK || errno == EAGAIN || errno == ECONNRESET) break;
				else throw std::runtime_error("SCTP recv failed, errno=" + std::to_string(errno));
			} else if (len == 0) {
				break;
			}
			...
			if (flags & MSG_NOTIFICATION) {
				...
				if (flags & MSG_EOR) {
					...
					if (infotype != SCTP_RECVV_RCVINFO) throw std::runtime_error("Missing SCTP recv info");
...
void SctpTransport::enqueueFlush() {
	if (mPendingFlushCount > 0) return;
...
bool SctpTransport::trySendQueue() {
	...
	while (auto next = mSendQueue.peek()) {
		...
		if (!trySendMessage(message)) return false;
...
bool SctpTransport::trySendMessage(message_ptr message) {
	...
	if (reliability.unordered) spa.sendv_sndinfo.snd_flags |= SCTP_UNORDERED;
	...
	ssize_t ret;
	if (!message->empty()) {
		ret = usrsctp_sendv(mSock, message->data(), message->size(), nullptr, 0, &spa, sizeof(spa), SCTP_SENDV_SPA, 0);
	} else {
		const char zero = 0;
	...
	if (message->type == Message::Binary || message->type == Message::String) mBytesSent += message->size();
	return true;
}
...
void SctpTransport::updateBufferedAmount(uint16_t streamId, ptrdiff_t delta) {
	...
	if (amount == 0) mBufferedAmount.erase(it);
...
void SctpTransport::handleUpcall() noexcept {
	try {
		...
		if (events & SCTP_EVENT_READ) enqueueRecv();

		if (events & SCTP_EVENT_WRITE) enqueueFlush();
...
int SctpTransport::handleWrite(byte *data, size_t len, uint8_t /*tos*/, uint8_t /*set_df*/) noexcept {
	try {
		std::unique_lock lock(mWriteMutex);
		PLOG_VERBOSE << "Handle write, len=" << len;

		if (!outgoing(make_message(data, data + len))) return -1;
	...
	switch (type) {
		...
		case SCTP_STREAM_RESET_EVENT: {
			...
			// IF_PLOG(plog::verbose) 
			{
				...
				else if (flags & SCTP_STREAM_RESET_OUTGOING_SSN) desc << "outgoing";
				else if (flags & SCTP_STREAM_RESET_INCOMING_SSN) desc << "incoming";
				else desc << "0";
...
size_t SctpTransport::bytesSent() {
	return mBytesSent;
}

size_t SctpTransport::bytesReceived() {
	return mBytesReceived;
}

optional<milliseconds> SctpTransport::rtt() {
	if (state() != State::Connected) return nullopt;

	struct sctp_status status = {};
	socklen_t len = sizeof(status);
	if (usrsctp_getsockopt(mSock, IPPROTO_SCTP, SCTP_STATUS, &status, &len)) return nullopt;
...
void SctpTransport::UpcallCallback(struct socket *, void *arg, int /* flags */) {
	...
	if (auto locked = Instances->lock(transport)) transport->handleUpcall();
}
