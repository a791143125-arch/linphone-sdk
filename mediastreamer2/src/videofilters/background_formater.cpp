...
static void boxBlurPlane(const uint8_t *src, int srcStride, uint8_t *dst, int dstStride,
                         int w, int h, int r, std::vector<uint8_t> &tmp) {
	if (r < 1 || w < 2 || h < 2) {
		for (int y = 0; y < h; ++y) memcpy(dst + y * dstStride, src + y * srcStride, w);
		return;
	}
	...
	void process(MSFilter *f) {
		...
		while ((m = ms_queue_get(f->inputs[0])) != nullptr) {
            ...
            if (mMode == MSBackgroundImage && mBgFrame) {
				...
				boxBlurPlane(src.planes[0], src.strides[0], dst.planes[0], dst.strides[0],
				             src.w, src.h, mRadius, mTmp);
				int rc = mRadius / 2 > 0 ? mRadius / 2 : 1;
				boxBlurPlane(src.planes[1], src.strides[1], dst.planes[1], dst.strides[1],
				             src.w / 2, src.h / 2, rc, mTmp);
				boxBlurPlane(src.planes[2], src.strides[2], dst.planes[2], dst.strides[2],
				             src.w / 2, src.h / 2, rc, mTmp);
...
static MSFilterMethod ms_background_replacer_methods[] = { {MS_BACKGROUND_FORMATER_SET_TYPE, ms_background_replacer_set_type},
