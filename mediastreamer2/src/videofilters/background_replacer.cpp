...
public:
	BackgroundReplacer(MSFilter *f) {
        ...
        char *path = ms_strdup_printf("%s/model.onnx",
                                      ms_factory_get_image_resources_dir(f->factory));
	...
	~BackgroundReplacer(){
        { std::lock_guard<std::mutex> lk(mMutex); mRunning = false; }
        mCv.notify_one();
        if (mWorker.joinable()) mWorker.join();   // ← évite le terminate
    }
	...
	void process(MSFilter *f) {
        ...
        while ((m = ms_queue_get(f->inputs[0])) != nullptr) {
            ...
            libyuv::I420ToARGB(pic.planes[0], pic.strides[0],
                               pic.planes[1], pic.strides[1],
                               pic.planes[2], pic.strides[2],
                               argb.data(), pic.w * 4, pic.w, pic.h);

            // PRODUCTEUR : publie la dernière image
            {
                std::lock_guard<std::mutex> lk(mMutex);
                mLastImg   = std::move(argb);
                mImgW = pic.w; mImgH = pic.h;
                mHasNewImg = true;
            }
            mCv.notify_one();

            // CONSOMMATEUR : récupère le dernier masque dispo (sans attendre)
            std::vector<float> mask; int mw = 0, mh = 0;
            {
                std::lock_guard<std::mutex> lk(mMutex);
                if (mHasResult) { mask = mLastResult; mw = maskW_; mh = maskH_; }
            }
            if (!mask.empty()) {
                const uint8_t Ybg = 0, Ubg = 128, Vbg = 128; 
...
private :
    ...
    std::vector<uint8_t> redimRGBA(const std::vector<uint8_t> &img, int srcW, int srcH,
    ...
    std::vector<float> redimMasque(const std::vector<float>& s,
    ...
    void inference_computer(){
        ...
        while (mRunning) {
            ...
            std::vector<uint8_t> img; int w, h;
            {
                ...
                if (!mRunning) break;
                img = std::move(mLastImg);            // prend la plus récente
                w = mImgW; h = mImgH;
                mHasNewImg = false;
            }
            ...
            // masque doux à la résolution modèle (proba premier plan)
            std::vector<float> mask(plane);
            for (int i = 0; i < plane; ++i)
                mask[i] = o[HUMAN_CH * plane + i];      

            if (lastImgBuffer.size() != mask.size())
                lastImgBuffer = mask;                            
            else
    ...
    // Redimensionnement nearest-neighbor d'un masque
