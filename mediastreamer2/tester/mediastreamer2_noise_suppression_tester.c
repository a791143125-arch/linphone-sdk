...
static void audio_convert_smff_to_wav(void) {

	bool_t wav_format = FALSE;
	bool_t smff_format = TRUE;
	if (wav_format) smff_format = FALSE;

	char *input_file = NULL;
	// if (smff_format) input_file = bc_tester_res("sounds/short_test.smff");
	if (smff_format) input_file = bc_tester_res("sounds/2025-06-18_11-55-13_floretest_cote_linux.smff");
	if (wav_format) input_file = bc_tester_res("sounds/nylon_48000_mono.wav");

	char *output_file = bc_tester_file("converted_audio.wav");
	unlink(output_file);

	int sample_rate = 48000;
	int nchannels = 1;
	int file_duration_ms = 0;
	int audio_done = 0;
	unsigned int filter_mask = FILTER_MASK_FILEPLAY | FILTER_MASK_SOUNDWRITE;
	MSFilter *player = NULL;
	MSFilter *sound_rec = NULL;
	MSFilter *tee_for_two = NULL;
	MSFilter *decoder = NULL;
	float waiting_time_ms = 180000;
	...
	// smff player
	if (smff_format) player = ms_factory_create_filter(msFactory, MS_SMFF_PLAYER_ID);

	if (ms_filter_call_method(player, MS_PLAYER_OPEN, input_file) == -1) {
		BC_FAIL("cannot open smff file");
		goto end;
	}

	// if (smff_format) {
	// 	int audio_start_ms = 90000;
	// 	ms_filter_call_method(player, MS_PLAYER_SEEK_MS, &audio_start_ms);
	// }

	ms_filter_call_method(player, MS_FILTER_GET_SAMPLE_RATE, &sample_rate);
	ms_filter_call_method(player, MS_FILTER_GET_NCHANNELS, &nchannels);
	ms_message("audio parameters read are: sample rate = %d Hz, mono/stereo = %d", sample_rate, nchannels);

	if (smff_format) {
		decoder = ms_factory_create_filter(msFactory, MS_OPUS_DEC_ID);
		ms_filter_call_method(decoder, MS_FILTER_SET_SAMPLE_RATE, &sample_rate);
		ms_filter_call_method(decoder, MS_FILTER_SET_NCHANNELS, &nchannels);
	}
	...
	if (ms_filter_call_method(ms_tester_soundwrite, MS_FILTER_SET_SAMPLE_RATE, &sample_rate) != 0) {
		...
		if (sample_rate != soundwrite_sample_rate) {
			BC_FAIL("not expected sampling rate");
			goto end;
		}
	}
	if (ms_filter_call_method(ms_tester_soundwrite, MS_FILTER_SET_NCHANNELS, &nchannels) != 0) {
		...
		if (nchannels != soundwrite_nchannels) {
			BC_FAIL("not expected channels number");
			goto end;
		}
	}
	...
	ms_filter_call_method(sound_rec, MS_FILTER_SET_SAMPLE_RATE, &sample_rate);
	ms_filter_call_method(sound_rec, MS_FILTER_SET_NCHANNELS, &nchannels);
	...
	if (wav_format) ms_connection_helper_link(&h, player, -1, 0);
	if (smff_format) {
		ms_connection_helper_link(&h, player, -1, 1);
	...
	ms_ticker_attach(ms_tester_ticker, player);
	ms_filter_call_method(player, MS_PLAYER_GET_DURATION, &file_duration_ms);
	ms_message("file duration is %d ms", file_duration_ms);
	printf("file duration is %fs\n", (float)file_duration_ms / 1000.);
	ms_filter_add_notify_callback(player, fileplay_eof, &audio_done, TRUE);
	...
	if (wav_format) ms_filter_call_method_noarg(player, MS_FILE_PLAYER_CLOSE); // wav player
	if (smff_format) ms_filter_call_method_noarg(player, MS_PLAYER_CLOSE);     // smff player
	ms_filter_call_method_noarg(sound_rec, MS_FILE_REC_CLOSE);
	ms_ticker_detach(ms_tester_ticker, player);

	ms_connection_helper_start(&h);
	if (wav_format) ms_connection_helper_unlink(&h, player, -1, 0);
	if (smff_format) {
		ms_connection_helper_unlink(&h, player, -1, 1);
...
static void
play_noisy_audio(char *noise_file, int sample_rate, float play_duration_ms, int start_noise_ms, float noise_gain) {
	char *talk_file;
	...
	char *output_file = bc_tester_file("output_audio.wav");
	unlink(output_file);
	...
	ms_filter_call_method(player_noise, MS_FILTER_GET_SAMPLE_RATE, &filter_rate);
	ms_filter_call_method(player_noise, MS_FILTER_GET_NCHANNELS, &filter_nchannels);
	if ((filter_nchannels != nchannels) || (filter_rate != sample_rate)) {
		resampler_noise = ms_factory_create_filter(msFactory, MS_RESAMPLE_ID);
		ms_filter_call_method(resampler_noise, MS_FILTER_SET_SAMPLE_RATE, &sample_rate);
		ms_filter_call_method(resampler_noise, MS_FILTER_SET_NCHANNELS, &nchannels);
		ms_message("resample noise audio at rate %d to get %d Hz", sample_rate, nchannels);
	}
	...
	ms_filter_call_method(player_talk, MS_FILTER_GET_SAMPLE_RATE, &filter_rate);
	ms_filter_call_method(player_talk, MS_FILTER_GET_NCHANNELS, &filter_nchannels);
	if ((filter_nchannels != nchannels) || (filter_rate != sample_rate)) {
		resampler_talk = ms_factory_create_filter(msFactory, MS_RESAMPLE_ID);
		ms_filter_call_method(resampler_talk, MS_FILTER_SET_SAMPLE_RATE, &sample_rate);
		ms_filter_call_method(resampler_talk, MS_FILTER_SET_NCHANNELS, &nchannels);
		ms_message("resample talk audio at rate %d to get %d Hz", sample_rate, nchannels);
	}

	mixer = ms_factory_create_filter(msFactory, MS_AUDIO_MIXER_ID);
	ms_filter_call_method(mixer, MS_FILTER_SET_SAMPLE_RATE, &sample_rate);
	...
	MSFilterDesc *ns_desc = ms_factory_lookup_filter_by_name(msFactory, "MSWebRTCNoiseSuppressor");
	noise_suppressor = ms_factory_create_filter_from_desc(msFactory, ns_desc);
	ms_filter_call_method(noise_suppressor, MS_FILTER_SET_SAMPLE_RATE, &sample_rate);

	tee_for_two = ms_factory_create_filter(msFactory, MS_TEE_ID);

	if (ms_filter_call_method(ms_tester_soundwrite, MS_FILTER_SET_SAMPLE_RATE, &sample_rate) != 0) {
		ms_filter_call_method(ms_tester_soundwrite, MS_FILTER_GET_SAMPLE_RATE, &filter_rate);
		if (sample_rate != filter_rate) {
			BC_FAIL("not expected sampling rate");
			goto end;
		}
	}
	if (ms_filter_call_method(ms_tester_soundwrite, MS_FILTER_SET_NCHANNELS, &nchannels) != 0) {
		ms_filter_call_method(ms_tester_soundwrite, MS_FILTER_GET_NCHANNELS, &filter_nchannels);
		if (nchannels != filter_nchannels) {
			BC_FAIL("not expected channels number");
			goto end;
		}
	}
	...
	ms_filter_call_method(sound_rec_with_noise, MS_FILTER_SET_SAMPLE_RATE, &sample_rate);
	ms_filter_call_method(sound_rec_with_noise, MS_FILTER_SET_NCHANNELS, &nchannels);
	...
	if (resampler_talk) ms_connection_helper_link(&h, resampler_talk, 0, 0);
	...
	ms_ticker_attach(ms_tester_ticker, player_noise);
	// ms_ticker_attach(ms_tester_ticker, player_talk); // test
	...
	ms_filter_call_method_noarg(sound_rec, MS_FILE_REC_CLOSE);
	ms_filter_call_method_noarg(sound_rec_with_noise, MS_FILE_REC_CLOSE);
	ms_ticker_detach(ms_tester_ticker, player_noise);
	// ms_ticker_detach(ms_tester_ticker, player_talk); // test
	...
	if (resampler_talk) ms_connection_helper_unlink(&h, resampler_talk, 0, 0);
...
static void talk_with_very_slight_noise_48kHz(void) {
	char *noise_file = bc_tester_res("sounds/echo_4_linux_medium_noise_1min_48kHz.wav");
	int rate_Hz = 48000;
	float play_duration_ms = 60000;
	int start_noise_ms = 0;
	float noise_gain = 0.1;
	play_noisy_audio(noise_file, rate_Hz, play_duration_ms, start_noise_ms, noise_gain);
	bctbx_free(noise_file);
}

static void talk_with_slight_noise_48kHz(void) {
	char *noise_file = bc_tester_res("sounds/echo_4_linux_medium_noise_1min_48kHz.wav");
	int rate_Hz = 48000;
	float play_duration_ms = 60000;
	int start_noise_ms = 0;
...
static void talk_with_medium_noise(void) {
...
static void talk_with_medium_noise_48kHz(void) {
	char *noise_file = bc_tester_res("sounds/echo_4_linux_medium_noise_1min_48kHz.wav");
	int rate_Hz = 48000;
	float play_duration_ms = 60000;
	int start_noise_ms = 0;
...
static void talk_with_increasing_noise(void) {
...
static void talk_with_increasing_noise_48kHz(void) {
...
static void talk_with_strong_increasing_noise(void) {
	char *noise_file = bc_tester_res("sounds/increasing_noise_16000Hz.wav");
	int rate_Hz = 16000;
	float play_duration_ms = 20000;
...
static test_t tests[] = {
    TEST_NO_TAG("Audio convert smff to wav", audio_convert_smff_to_wav),
    TEST_NO_TAG("Talk with very slight noise 48kHz", talk_with_very_slight_noise_48kHz),
    TEST_NO_TAG("Talk with slight noise 48kHz", talk_with_slight_noise_48kHz),
    TEST_NO_TAG("Talk with medium noise", talk_with_medium_noise),
    TEST_NO_TAG("Talk with medium noise 48kHz", talk_with_medium_noise_48kHz),
    TEST_NO_TAG("Talk with increasing noise", talk_with_increasing_noise),
    TEST_NO_TAG("Talk with increasing noise 48kHz", talk_with_increasing_noise_48kHz),
    TEST_NO_TAG("Talk with strong increasing noise", talk_with_strong_increasing_noise),
};
