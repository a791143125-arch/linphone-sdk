...
class AudioAnalysisNoiseSuppression:
    ...
    def get_results(self, log_file):
        """
        Extracts the results of the AEC filtering during a test from mediastreamer2 test suite Noise Suppression in
        audio.
        ...
        with open(log_file) as f:
            ...
            for i, line in enumerate(contents):
                ...
                if "Max cross-correlation obtained on speech parts" in line and self.similarity_in_speech == 0:
                ...
                if "Energy measured on silences" in line:
                    ...
                    if match:
                        if self.energy_in_silence == 0.:
                            self.energy_in_silence = float(match.group(1))
                        elif self.energy_in_silence_with_noise == 0.:
                            self.energy_in_silence_with_noise = float(match.group(1))
                ...
                if "Suite [Noise suppression in audio] Test [" in line:
    ...
    def align_on_speech(self):
        ...
        self.align_on_reference(self.speech, self.noisy_speech)
        print("-- Align clean audio on initial speech --")
        self.align_on_reference(self.speech, self.clean)


    def align_on_reference(self, reference, audio):
        """
        Align the audio with the reference on the maximal correlation.
        """

        if reference.data is not None and audio.data is not None:

            max_pos_samples, corr = self.compute_correlation(reference.data[self.start_sample:],
                                                             audio.data[self.start_sample:])
    ...
    def compute_energy_difference_with_speech(self):
        ...
        if self.noisy_speech.silence is None:
            ...
            energy_in_silence_noisy_speech = np.sum(np.abs(self.noisy_speech.silence)) - np.sum(
                np.abs(self.speech.silence))
        print(f"energy measured on silence of noisy audio: {energy_in_silence_noisy_speech:1.1f}")
    ...
    def compute_denoising_quality(self, log_file, start_analysis_time_ms=0, plot_silence_and_talk=True, noise=None):
        ...
        self.start_sample = int(float(self.start_analysis_time_ms) / 1000.) * self.speech.sample_rate_hz
    ...
    def truncate_speech(self):
        """
        If the audio sizes are too much different, the speech audio is shortened in order to reduce computation time for
