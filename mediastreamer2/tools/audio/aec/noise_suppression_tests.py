...
class NoiseSuppressionTest:
    ...
    def __init__(self, test_name, **kwargs):
        ...
        self.suite = "Noise suppression in audio"
    ...
    def move_files(self):
        """Move the files generated by the test to another folder. Make sure that the output files are not deleted at the
        end of the test. Each output file is the latest wav file starting with either clean_audio or noisy_audio."""

        for audio_type in ["clean_audio_", "noisy_audio_"]:
            audio_file_list = glob.glob(audio_type + self.test_name + "*.wav")
            ...
            if len(audio_file_list) > 0:
                ...
                if audio_type == "noisy_audio_":
                    file_name = self.files.noisy_speech.file_name
                else:
