...
static void check_uri_and_headers(belle_sip_message_t *message) {
	if (belle_sip_message_is_request(message)) {
		BC_ASSERT_TRUE(belle_sip_request_get_uri(BELLE_SIP_REQUEST(message)) ||
...
static void testInviteMessage(void) {
	const char *raw_message =
	    "INVITE sip:becheong@sip.linphone.org SIP/2.0\r\n"
	    "Via: SIP/2.0/UDP "
	...
	BC_ASSERT_PTR_NOT_NULL(header_support =
	                           BELLE_SIP_HEADER_SUPPORTED(belle_sip_message_get_header(message, "Supported")));
	if (header_support) {
...
static void testInviteMessageWithTelUri(void) {
	const char *raw_message =
	    "INVITE tel:11234567888;phone-context=vzims.fr SIP/2.0\r\n"
	    "Via: SIP/2.0/UDP "
	    "10.23.17.117:22600;branch=z9hG4bK-d8754z-4d7620d2feccbfac-1---d8754z-;rport=4820;received=202.165.193.129\r\n"
	    "Max-Forwards: 70\r\n"
	    "Contact: <sip:bcheong@202.165.193.129:4820>\r\n"
	...
	BC_ASSERT_PTR_NOT_NULL(belle_sip_message_get_header(message, "Content-Type"));
	check_uri_and_headers(message);
	BC_ASSERT_PTR_NOT_NULL(belle_sip_header_address_get_absolute_uri(
	    BELLE_SIP_HEADER_ADDRESS(belle_sip_message_get_header_by_type(message, belle_sip_header_from_t))));
	BC_ASSERT_PTR_NOT_NULL(belle_sip_header_address_get_absolute_uri(
	    BELLE_SIP_HEADER_ADDRESS(belle_sip_message_get_header_by_type(message, belle_sip_header_to_t))));
	BC_ASSERT_STRING_EQUAL(belle_generic_uri_get_opaque_part(belle_sip_request_get_absolute_uri(request)),
	                       "11234567888;phone-context=vzims.fr");
	belle_sip_object_unref(message);
	belle_sip_free(encoded_message);
}
...
static void test401Response(void) {
	const char *raw_message =
	    ...
	    "realm=\"sip.ovh.net\",nonce=\"24212965507cde726e8bc37e04686459\",opaque=\"241b9fb347752f2\",stale=false,"
	    "algorithm=MD5\r\n"
	    "Content-Length: 0\r\n\r\n";
...
void channel_parser_tester_recovery_from_error_base(const char *prelude, const char *raw_message) {
	...
	belle_sip_channel_t *channel =
	    belle_sip_stream_channel_new_client(stack, NULL, 45421, NULL, "127.0.0.1", 45421, TRUE);
...
static void testMalformedFrom(void) {
	...
	belle_sip_listening_point_t *lp =
	    belle_sip_stack_create_listening_point(stack, "127.0.0.1", LISTENING_POINT_PORT, "tcp");
	...
	belle_sip_object_ref(message); /* double ref: originally the message is created with 0 refcount, and
	                                  dispatch_message will unref() it.*/
...
static void testMalformedMandatoryField(void) {
	...
	const char *raw_message =
	    "MESSAGE sip:lollol.iphone@22.22.222.222:5861;transport=tcp SIP/2.0\r\n"
	    ...
	    "To: "
	    "<sip:lollol-labo-iphone4s@22.22.22.222:5861;app-id=fr.lollol.phone.prod;pn-type=apple;pn-tok="
	    "azertyuiopqsdfghhjjkmlqoijfubieuzhqiluehcpoqidufqsdkjlcnuoishcvs;pn-msg-str=IM_MSG;pn-call-str=IC_MSG;pn-call-"
	    "snd=ring.caf;pn-msg-snd=msg.caf;transport=tcp>;tag=\r\n"
	    ...
	    "<VDUCMediaConfig GId=\"1234567\" IdPosteType=\"123\"><Label>Salut Bilout TFT MS2</Label><MediaConfig "
	    "GId=\"456\"><CommandCode Code=\"MediaCommand*\"><Label>Porte ouverte</Label></CommandCode><withVideo "
	    "FPS=\"0.0\"/></MediaConfig></VDUCMediaConfig>\r\n"
	    "\r\n";
...
static void testRFC2543_base(char *branch) {
	...
	belle_sip_object_ref(message);
	belle_sip_object_ref(message); /*yes double ref: originally the message is created with 0 refcount, and
	                                  dispatch_message will unref() it.*/
	...
	BC_ASSERT_PTR_NOT_NULL(
	    belle_sip_provider_find_matching_server_transaction(prov, request)); /*make sure branch id is properly set*/
...
static void testUriHeadersInInvite(void) {
	...
	request = belle_sip_request_create(belle_sip_uri_parse(raw_uri), "INVITE", belle_sip_provider_create_call_id(prov),
	                                   belle_sip_header_cseq_create(20, "INVITE"),
	                                   belle_sip_header_from_create2("sip:toto@titi.com", "4654"), NULL,
	...
	if (raw_header) {
		BC_ASSERT_STRING_EQUAL(belle_sip_header_extension_get_value(BELLE_SIP_HEADER_EXTENSION(raw_header)),
		                       "blue;weftutu=bla");
	}
	...
	BC_ASSERT_STRING_NOT_EQUAL(
	    belle_sip_header_get_unparsed_value(belle_sip_message_get_header(BELLE_SIP_MESSAGE(request), "From")), "toto");
...
static void testUrisComponentsForRequest(void) {
	...
	request = belle_sip_request_create(
	    belle_sip_uri_parse(raw_uri), "INVITE", belle_sip_provider_create_call_id(prov),
	    belle_sip_header_cseq_create(20, "INVITE"), belle_sip_header_from_create2("sip:toto@titi.com", "4654"),
	    belle_sip_header_to_parse("To: sip:titi@titi.com:5061"), belle_sip_header_via_new(), 70);
	BC_ASSERT_PTR_NOT_NULL(request);
