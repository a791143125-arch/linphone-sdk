...
belle_sip_message_t *belle_sip_message_parse_sipfrag(const char *value) {
	size_t message_length;
	auto parser = bellesip::SIP::Parser::getInstance();
	auto object = parser->parse(value, "sipfrag", &message_length);
	if (object) {
		auto context = BELLE_SIP_PARSER_CONTEXT(object);
		belle_sip_message_t *message = reinterpret_cast<belle_sip_message_t *>(context->obj);
		belle_sip_object_unref(context);
...
extern "C" void belle_sip_message_add_header(belle_sip_message_t *message, belle_sip_header_t *header) {
	headers_container_t *headers_container = get_or_create_container(message, belle_sip_header_get_name(header));
	headers_container->header_list =
...
void belle_sip_message_add_header_from_parser_context(belle_sip_message_t *message,
	...
	if (context->obj) {
		belle_sip_header_t *header = reinterpret_cast<belle_sip_header_t *>(context->obj);
		belle_sip_message_add_header(message, header);
	}
	belle_sip_object_unref(context);
}
...
belle_sip_object_t *_belle_sip_message_get_header_by_type_id(const belle_sip_message_t *message,
	...
	for (e1 = message->header_list; e1 != NULL; e1 = e1->next) {
		...
		if (headers_container->header_list) {
			belle_sip_object_t *ret = reinterpret_cast<belle_sip_object_t *>(headers_container->header_list->data);
...
belle_sip_request_t *belle_sip_try_request_parse(const char *value) {
	auto parser = bellesip::SIP::Parser::getInstance();
	size_t parsedSize = 0;
	auto result = reinterpret_cast<belle_sip_request_t *>(parser->parse(value, "request", &parsedSize, false));
	return result;
}
belle_sip_request_t *belle_sip_request_parse(const char *value) {
	belle_sip_request_t *l_parsed_object = belle_sip_try_request_parse(value);
	if (!l_parsed_object ||
	    (!belle_sip_request_get_uri(l_parsed_object) && !belle_sip_request_get_absolute_uri(l_parsed_object))) {
		// The request could not be parsed or there was an error parsing the request-uri, so the parsed request is not
		// valid
		belle_sip_error("request parser error for [%s]", value);
		if (l_parsed_object) belle_sip_object_unref(l_parsed_object);
		return nullptr;
	}
